...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Function{insertHead}{\(l\) : BaseNIntegerList, value : \(Array<Character>\), valSize : Integer} : BaseNIntegerList
            ...
            \State size(new\_el) \(\leftarrow\) valSize
...
\emph{sumBaseNIntegers}: \(Array<Character>\) \(×\) \emph{Integer} \(×\) \(Array<Character>\) \(×\) \emph{Integer} \(×\) \emph{Integer} \(\rightarrow\) \emph{\(Array<Character>\)},
sums two integers defined in the given base (without any conversion).
The second and the fourth parameters are the integers' size.
The fifth parameter is the integers' base.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Function{sumBaseNIntegers}{\(a\) : \(Array<Character>\), sizeA : \emph{Integer}, \(b\) : \(Array<Character>\), sizeB : \emph{Integer}, base : \emph{Integer}} : \(Array<Character>\)
            ...
            \If{remainder \(>\) 0}
                \State \(temp\_result \leftarrow createArray<Character>(sizeC + 1)\)
                \State \(temp\_result_0 \leftarrow\) remainder
                \State memcpy(temp\_result + 1, result, sizeC \(×\) size(char))
...
\emph{sumIntegerList}: \emph{BaseNIntegerList} \(\rightarrow\) \(Array<Character>\),
sums all the integers defined in the specified list using the binary addition and returns
the corresponding results as an integer (\emph{\(Array<Character>\)}) represented in the base of the list.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Function{sumIntegerList}{\(l\) : BaseNIntegerList} : \(Array<Character>\)
            \State \(Array<Character>\) sum \(\leftarrow createArray<Character>(0)\)
...
\emph{baseNToDecimal}: \(Array<Character> ×\) Integer \(×\) Integer \(\rightarrow\) \emph{Integer},
converts the specified integer (\(Array<Character>\) represented with the specified base (Integer, third parameter)) into a corresponding integer in base 10. The second parameter is the integer's size.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Function{baseNToDecimal}{n : \(Array<Character>\), valSize : integer, base : integer} : integer
            \State number \(\leftarrow\) 0
            \State i \(\leftarrow\) valSize - 1
            \State maxWeight \(\leftarrow\) valSize - 1
            \While{i \(>\) 0}
                \State number \(\leftarrow\) number + \(n_i\) * \(base^{maxWeight - i}\)
                \State i \(\leftarrow\) i - 1
            \EndWhile
...
\emph{decimalToBaseN}: \emph{Integer} \(\rightarrow\) \emph{\(Array<Character>\)},
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Function{decimalToBaseN}{n : integer, base : integer} : \(Array<Character>\)
            ...
            \Repeat
                ...
                \State i \(\leftarrow\) i - 1;
            \Until{i \(>=\) 0}
            \State decimalToBaseN \(\leftarrow\) array
        \EndFunction
    \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Function{convertBaseToBinary}{n : \(Array<Character>\), base : integer} : \(Array<Character>\)
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Function{convertBinaryToBase}{n : \(Array<Character>\), base : integer} : \(Array<Character>\)
