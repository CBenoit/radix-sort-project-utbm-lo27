...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Function{sumBaseNIntegers}{\(a\) : \emph{BigInteger}, \(b\) : \emph{BigInteger}, base : \emph{Integer}} : \emph{BigInteger}
            ...
            \State BigInteger result \(\leftarrow\) createBigInteger(array, sizeC)
...
adds a new integer (\emph{BigInteger}) at the end of the specified list in bucket N (N being the third parameter between 0 and base of the integers less 1).
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Function{addIntegerIntoBucket}{lol : \emph{BaseNIntegerListOfList}, v : \emph{BigInteger}, N : \emph{Integer}} : \emph{BaseNIntegerListOfList}
            \State // assert(\(N < base(lol)\))
            \State array(lol)\(_N\) \(\leftarrow\) insertTail(array(lol)\(_N\), v)
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Function{buildBucketList}{l : \emph{BaseNIntegerList}, position : \emph{Integer}} : \emph{BaseNIntegerListOfList}
            ...
            \While{next(el) != undefined}
                \State lol \(\leftarrow\) addIntegerIntoBucket(lol, value(el), position)
                \State el \(\leftarrow\) next(el)
            \EndWhile
    ...
    \begin{algorithmic}
        \Function{buildIntegerList}{lol : \emph{BaseNIntegerListOfList}} : \emph{BaseNIntegerList}
            ...
            \For{i from 0 to base(lol) - 1}
                ...
                \While{el != undefined}
                    \State l \(\leftarrow\) insertTail(l, value(el))
                    \State el \(\leftarrow\) next(el)
                \EndWhile
            \EndFor
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Function{deleteBucketList}{lol : \emph{BaseNIntegerListOfList}} : \(\emptyset\)
            \For{i from 0 to base(lol) - 1}
                \State // Doesn't use deleteIntegerList because it would also
                \State // free the BigInteger's value that should be still used by other lists' BigInteger.
                \State free(array(lol)\(_i\))
            \EndFor
